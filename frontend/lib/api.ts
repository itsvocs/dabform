...
export const patientenApi = {
  ...
  search: (query: string) => {
    ...
    return fetchApi<Patient[]>(`/api/patienten?search=${encodeURIComponent(query)}`);
  },
};
...
export const krankenkassenApi = {
  ...
  search: (query: string) => {
    if (!query || query.length < 2) return Promise.resolve([]);
    return fetchApi<Krankenkasse[]>(`/api/krankenkassen?search=${encodeURIComponent(query)}`);
  },
};
...
export const unfallbetriebeApi = {
  ...
  search: (query: string) => {
    if (!query || query.length < 2) return Promise.resolve([]);
    return fetchApi<Unfallbetrieb[]>(`/api/unfallbetriebe?search=${encodeURIComponent(query)}`);
  },
};
...
export const uvTraegerApi = {
  ...
  search: (query: string) => {
    if (!query || query.length < 2) return Promise.resolve([]);
    return fetchApi<UVTraeger[]>(`/api/uv-traeger?search=${encodeURIComponent(query)}`);
  },
};
...
export const pdfApi = {
  ...
  downloadAndSave: async (berichtId: number, typ: 'uv' | 'kk' = 'uv', filename?: string) => {
